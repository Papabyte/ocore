...
//		throw Error("conn but not batch");
	...
	initConnection(function(subBatch){
		...
		for (var i=0; i<objValidationState.arrAdditionalQueries.length; i++){
			...
			subBatch.sql.addQuery(arrQueries, objAdditionalQuery.sql, objAdditionalQuery.params);
		...
		if (bCordova)
			subBatch.sql.addQuery(arrQueries, "INSERT INTO joints (unit, json) VALUES (?,?)", [objUnit.unit, JSON.stringify(objJoint)]);
		...
		if (conf.bLight){
			...
			values += ",?,"+subBatch.sql.getFromUnixTime("?");
		...
		var ignore = (objValidationState.sequence === 'final-bad') ? subBatch.sql.getIgnore() : ''; // possible re-insertion of a previously stripped unit
		subBatch.sql.addQuery(arrQueries, "INSERT " + ignore + " INTO units ("+fields+") VALUES ("+values+")", params);
		
		if (objJoint.ball && !conf.bLight){
			subBatch.sql.addQuery(arrQueries, "INSERT INTO balls (ball, unit) VALUES(?,?)", [objJoint.ball, objUnit.unit]);
			subBatch.sql.addQuery(arrQueries, "DELETE FROM hash_tree_balls WHERE ball=? AND unit=?", [objJoint.ball, objUnit.unit]);
			...
			if (objJoint.skiplist_units)
				...
				subBatch.sql.addQuery(arrQueries, "INSERT INTO skiplist_units (unit, skiplist_unit) VALUES (?,?)", [objUnit.unit, objJoint.skiplist_units[i]]);
		}
		
		if (objUnit.parent_units){
			for (var i=0; i<objUnit.parent_units.length; i++)
			subBatch.sql.addQuery(arrQueries, "INSERT INTO parenthoods (child_unit, parent_unit) VALUES(?,?)", [objUnit.unit, objUnit.parent_units[i]]);
		}
		
		var bGenesis = storage.isGenesisUnit(objUnit.unit);
		if (bGenesis)
			subBatch.sql.addQuery(arrQueries, 
				"UPDATE units SET is_on_main_chain=1, main_chain_index=0, is_stable=1, level=0, witnessed_level=0 \n\
				WHERE unit=?", [objUnit.unit]);
		else {
			subBatch.sql.addQuery(arrQueries, "UPDATE units SET is_free=0 WHERE unit IN(?)", [objUnit.parent_units], function(result){
		...
		if (Array.isArray(objUnit.witnesses)){
			for (var i=0; i<objUnit.witnesses.length; i++){
				...
				subBatch.sql.addQuery(arrQueries, "INSERT INTO unit_witnesses (unit, address) VALUES(?,?)", [objUnit.unit, address]);
			}
			subBatch.sql.addQuery(arrQueries, "INSERT "+subBatch.sql.getIgnore()+" INTO witness_list_hashes (witness_list_unit, witness_list_hash) VALUES (?,?)", 
		...
		for (var i=0; i<objUnit.authors.length; i++){
			...
			if (definition){
				...
				subBatch.sql.addQuery(arrQueries, "INSERT "+subBatch.sql.getIgnore()+" INTO definitions (definition_chash, definition, has_references) VALUES (?,?,?)", 
			...
			if (bGenesis)
				subBatch.sql.addQuery(arrQueries, "UPDATE unit_authors SET _mci=0 WHERE unit=?", [objUnit.unit]);
		...
		if (!objUnit.content_hash){
			for (var i=0; i<objUnit.messages.length; i++){
				...
				subBatch.sql.addQuery(arrQueries, "INSERT INTO messages \n\
				...
				if (message.payload_location === "inline"){
					switch (message.app){
						...
						case "poll":
							...
							subBatch.sql.addQuery(arrQueries, "INSERT INTO polls (unit, message_index, question) VALUES(?,?,?)", [objUnit.unit, i, poll.question]);
							for (var j=0; j<poll.choices.length; j++)
								subBatch.sql.addQuery(arrQueries, "INSERT INTO poll_choices (unit, choice_index, choice) VALUES(?,?,?)", 
									[objUnit.unit, j, poll.choices[j]]);
							break;
						case "vote":
							var vote = message.payload;
							subBatch.sql.addQuery(arrQueries, "INSERT INTO votes (unit, message_index, poll_unit, choice) VALUES (?,?,?,?)", 
								[objUnit.unit, i, vote.unit, vote.choice]);
							break;
						case "attestation":
							var attestation = message.payload;
							subBatch.sql.addQuery(arrQueries, "INSERT INTO attestations (unit, message_index, attestor_address, address) VALUES(?,?,?,?)", 
						...
						case "asset":
							...
							subBatch.sql.addQuery(arrQueries, "INSERT INTO assets (unit, message_index, \n\
						...
						function(input, j, cb3){
							...
							determineInputAddress(function(address){
								var is_unique = 
									...
									? null : 1;
								subBatch.sql.addQuery(arrQueries, "INSERT INTO inputs \n\
								...
								switch (type){
									...
									case "witnessing":
										...
										subBatch.sql.addQuery(arrQueries, "UPDATE "+table+" SET is_spent=1 \n\
		...
		function updateBestParent(cb){
			...
			subBatch.sql.query(
				...
				function(rows){
					...
					subBatch.sql.query("UPDATE units SET best_parent_unit=? WHERE unit=?", [my_best_parent_unit, objUnit.unit], function(){ cb(); });
				}
			);
		}
		...
		function determineMaxLevel(handleMaxLevel){
			...
			async.each(
				...
				function(parent_unit, cb){
					storage.readStaticUnitProps(subBatch.sql, parent_unit, function(props){
		...
		function updateLevel(cb){
			subBatch.sql.cquery("SELECT MAX(level) AS max_level FROM units WHERE unit IN(?)", [objUnit.parent_units], function(rows){
				...
				determineMaxLevel(function(max_level){
					...
					subBatch.sql.query("UPDATE units SET level=? WHERE unit=?", [rows[0].max_level + 1, objUnit.unit], function(){
						cb();
					});
				});
			});
		...
		function updateWitnessedLevelByWitnesslist(arrWitnesses, cb){
			...
			function setWitnessedLevel(witnessed_level){
				...
				subBatch.sql.query("UPDATE units SET witnessed_level=? WHERE unit=?", [witnessed_level, objUnit.unit], function(){
			...
			function addWitnessesAndGoUp(start_unit){
				...
				profiler.start();
				storage.readStaticUnitProps(subBatch.sql, start_unit, function(props){
					...
					profiler.start();
					storage.readUnitAuthors(subBatch.sql, start_unit, function(arrAuthors){
		...
		mutex.lock(["write"], function(unlock){
			...
			var batch = bCordova ? null : subBatch.kv;
			if (bGenesis){
					...
					if (objUnit.parent_units){
						if (!conf.bLight){
							if (objValidationState.bAA) {
								...
								if (arrAADefinitionPayloads.length > 0) {
									arrOps.push(function (cb) {
										...
										storage.insertAADefinitions(subBatch.sql, arrAADefinitionPayloads, objUnit.unit, objValidationState.initial_trigger_mci, true, cb);
									});
								}
							}
							if (!conf.bFaster)
							...
							arrOps.push(function(cb){
								...
								main_chain.updateMainChain(subBatch.sql, batch, null, objUnit.unit, objValidationState.bAA, cb);
							});
						}
						if (preCommitCallback)
							arrOps.push(function(cb){
								console.log("executing pre-commit callback");
								preCommitCallback(subBatch.sql, cb);
							});
					}
					var arrOpstime = Date.now();

					async.series(arrOps, function(err){
						console.log("arrOps took " + (Date.now()-arrOpstime ));
						...
						saveToKvStore(function(){
							...
							function onCommit(){
								...
								if (err) {
									...
									storage.resetMemory(subBatch, function(){
										unlock();
								//		if (!bInLargerTx)
								//			conn.release();
									});
